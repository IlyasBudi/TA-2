...
<body class="min-h-screen bg-dots-darker bg-center bg-gray-100 dark:bg-dots-lighter dark:bg-gray-900 selection:bg-red-500 selection:text-white flex items-center justify-center p-6">
  <div class="w-full max-w-2xl bg-white dark:bg-gray-800/50 dark:bg-gradient-to-bl from-gray-700/50 via-transparent dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-lg shadow-2xl shadow-gray-500/20 dark:shadow-none p-8">
    <div class="flex items-center gap-4 mb-6">
      <div class="w-12 h-12 rounded-lg bg-red-50 dark:bg-red-800/20 flex items-center justify-center">
        <svg viewBox="0 0 62 65" fill="none" xmlns="http://www.w3.org/2000/svg" class="h-6 w-auto">
      ...
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Buat Akun Penyewa</h1>
        <p class="text-gray-600 dark:text-gray-400">Masukkan data di bawah ini.</p>
      </div>
    </div>
    ...
    <form method="POST" action="{{ route('do.penyewaregister') }}" class="grid sm:grid-cols-2 gap-5">
      ...
      <div>
        ...
        <input name="phone_number" value="{{ old('phone_number') }}" required
               class="mt-1 w-full rounded-lg border border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:border-red-500 focus:ring-red-500 focus:ring-1 p-3"/>
        @error('phone_number') <p class="text-sm text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</p> @enderror
      </div>
      ...
      <div class="sm:col-span-2">
        ...
        <input name="address" value="{{ old('address') }}" required
               class="mt-1 w-full rounded-lg border border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:border-red-500 focus:ring-red-500 focus:ring-1 p-3"/>
        @error('address') <p class="text-sm text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</p> @enderror
      </div>
    ...
    <p class="mt-6 text-sm text-center text-gray-600 dark:text-gray-400">
      Sudah punya akun? <a href="{{ route('login') }}" class="text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">Masuk</a>
    </p>
  </div>
</body>
</html>
