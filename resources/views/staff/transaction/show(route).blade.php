...
@push('after-scripts')
    <script>
        ...
        var data{{ $transaction->user->id }} = L.layerGroup();
        ...
        var map = L.map('map', {
            ...
            zoom: 10,
            layers: [dark, data{{ $transaction->user->id }}]
        });
        ...
        // mengatur overlayers
        var overlays = {
            "{{ $transaction->kantorcabang->name }}": data{{ $transaction->kantorcabang->id }},
        };

        L.control.layers(baseLayers, overlays).addTo(map);
        L.marker([{{ $transaction->latitude }}, {{ $transaction->longitude }}]).bindPopup(
            "<div class='my-2'><img src='{{ Storage::url($transaction->bus->image) }}' class='img-fluid'></div>" +
            "<div class='my-2'><strong>Nama Penyewa:</strong> <br>{{ $transaction->user->name }}</div>" +
            "<div class='my-2'><a href='/bus/{{ $transaction->bus->id }}' class='btn btn-outline-success btn-sm'>Detail Bus</a></div>"
        ).addTo(map);
        ...
        function getPosition(position) {
            ...
            L.Routing.control({
                waypoints: [
                    ...
                    L.latLng({{ $transaction->latitude }}, {{ $transaction->longitude }}),
                ],
